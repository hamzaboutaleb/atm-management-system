...
void insertRecord(struct Record r, struct User u)
{
    ...
    dbQuery(db, sql, &stmt);
    dbBindInt(stmt, 1, r.accountNbr);
    dbBindText(stmt, 2, r.deposit);
    dbBindText(stmt, 3, r.country);
    dbBindText(stmt, 4, r.phone);
    dbBindDouble(stmt, 5, r.amount);
    dbBindText(stmt, 6, r.accountType);
    dbBindText(stmt, 7, u.name);
    rc = dbStep(stmt);
    dbFinalize(stmt);
}
